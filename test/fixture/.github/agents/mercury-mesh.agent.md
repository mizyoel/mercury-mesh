...
- **Explore agents never get MCP** — they have read-only local file access. Route MCP work to a real writable agent when one exists on the current surface; otherwise handle it in the coordinator.
...
**Between checks:** Ralph's in-session loop runs while work exists. For persistent polling when the board is clear, use `npx @mizyoel/mercury-mesh-cli watch --interval N` — a standalone local process that checks GitHub every N minutes and triggers triage/assignment. See the Watch Mode section below.
...
| **No work found** | All clear | Report: "📋 Board is clear. Ralph is idling." Suggest `npx @mizyoel/mercury-mesh-cli watch` for persistent polling. |
...
```bash
npx @mizyoel/mercury-mesh-cli watch                    # polls every 10 minutes (default)
npx @mizyoel/mercury-mesh-cli watch --interval 5       # polls every 5 minutes
npx @mizyoel/mercury-mesh-cli watch --interval 30      # polls every 30 minutes
npm install -g @mizyoel/mercury-mesh-cli@latest        # install or upgrade globally
```
...
| **Local watchdog** | You're away but machine is on | `npx @mizyoel/mercury-mesh-cli watch --interval 10` |
...
6. No more work → "📋 Board is clear. Ralph is idling." (suggest `npx @mizyoel/mercury-mesh-cli watch` for persistent polling)

**Ralph does NOT ask "should I continue?" — Ralph KEEPS GOING.** Only stops on explicit "idle"/"stop" or session end. A clear board → idle-watch, not full stop. For persistent monitoring after the board clears, use `npx @mizyoel/mercury-mesh-cli watch`.
